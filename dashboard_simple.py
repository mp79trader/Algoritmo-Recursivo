...
class TerminalDashboardSimple:
    def __init__(self):
        self.console = Console()
        
    def show_header(self):
        header_text = Text("ALGORITMO FFT RECURSIVO", style="bold cyan")
        sub_text = Text("Para Predicci�n de Mercado - Futuros y Acciones", style="italic white")
    ...
    def show_main_menu(self):
        ...
        self.console.print("1. 📊 Analizar un nico instrumento (Futuro/Acci�n)")
        self.console.print("2. 📈 An�lisis comparativo de m�ltiples futuros")
        self.console.print("3. 📋 Lista de s�mbolos disponibles")
        self.console.print("4. ⚙️ Configuraci�n")
        ...
        menu_map = {
            "1": "📊 Analizar un nico instrumento (Futuro/Acci�n)",
            "2": "📈 An�lisis comparativo de m�ltiples futuros",
            "3": "📋 Lista de s�mbolos disponibles",
            "4": "⚙️ Configuraci�n",
            "5": "📊 Ver resultados anteriores",
            "6": "🚪 Salir"
        }
        
        return menu_map.get(choice, "6")
    ...
    def show_symbol_selection(self, symbol_type="all"):
        ...
        self.console.print("\n[bold cyan]SELECCI�N DE S�MBOLO[/bold cyan]\n")
        
        all_symbols = []
        if symbol_type == "all":
            for sym in symbols['futures']:
                all_symbols.append(f"{sym} - {Config.FUTURES_SYMBOLS[sym]}")
            for sym in symbols['stocks']:
                all_symbols.append(f"{sym} - {Config.STOCK_SYMBOLS[sym]}")
        
        for i, sym in enumerate(all_symbols, 1):
            self.console.print(f"{i}. {sym}")
        
        choice = input("\nIngresa el s�mbolo (ej. MNQ=F, AAPL): ").strip().upper()
            ...
            for step in steps:
                ...
                if step['action']:
                    try:
                        ...
                        self.console.print(f"[red]Error en paso {step['desc']}: {str(e)}[/red]")
                progress.advance(task)
    
    def show_symbol_info_table(self, symbol_info, data):
        table = Table(title="Informaci�n del Instrumento", show_header=True, header_style="bold magenta")
        table.add_column("Propiedad", style="cyan", width=20)
        table.add_column("Valor", style="white")
        
        table.add_row("S�mbolo", symbol_info['ticker'])
        table.add_row("Nombre", symbol_info['name'])
        table.add_row("Tipo", symbol_info['type'])
        table.add_row("Puntos de datos", str(len(data['close'])))
        table.add_row("Precio M�nimo", f"${data['close'].min():.2f}")
        table.add_row("Precio M�ximo", f"${data['close'].max():.2f}")
        table.add_row("Precio Actual", f"${data['close'][-1]:.2f}")
        
        self.console.print(table)
    
    def show_prediction_table(self, prediction, config):
        table = Table(title="Predicciones para Pr�ximos " + str(config['prediction_days']) + " D�as", 
                     show_header=True, header_style="bold green")
        table.add_column("D�a", style="cyan", width=8)
    ...
    def show_trend_analysis(self, analysis):
        ...
        trend_table = Table(title="An�lisis de Tendencia", show_header=True, header_style="bold yellow")
        trend_table.add_column("Indicador", style="cyan", width=20)
        trend_table.add_column("Valor", style="white")
        
        trend_table.add_row("Direcci�n", f"[{trend_style}]{trend['direction']}[/]")
    ...
    def show_cycles_table(self, cycles_data):
        table = Table(title="An�lisis de Ciclos Detectados", show_header=True, header_style="bold blue")
        table.add_column("Rank", style="cyan", width=6)
        table.add_column("Per�odo (d�as)", style="white", width=15)
        ...
        if cycles_data['dominant_cycle']:
            ...
            dom_panel = Panel(
                f"Ciclo Dominante: Per�odo = {dom['period']:.2f} d�as, Frecuencia = {dom['frequency']:.4f}",
    ...
    def show_spectral_summary(self, spectral):
        ...
        table.add_row("Magnitud M�xima", f"{max_mag:.4f}")
    ...
    def show_summary_dashboard(self, symbol_info, analysis, prediction, data):
        ...
        summary.add_row("S�mbolo:", symbol_info['ticker'])
        ...
        summary.add_row("Ciclo Dominante:", 
                       f"{analysis['cycles']['dominant_cycle']['period']:.1f} d�as" if analysis['cycles']['dominant_cycle'] else "N/A")
        ...
        layout["details"].update(Panel(prediction_summary, title="🔮 Predicci�n", border_style="yellow"))
    ...
    def show_visualization_menu(self, results_files):
        ...
        self.console.print("3. ⬅️ Volver al men� principal\n")
        ...
        return choice_map.get(choice, "3")
    ...
    def open_visualizations(self, results_files):
        ...
        for file_path in results_files:
            ...
            try:
                ...
                self.console.print(f"[red]Error abriendo {file_path}: {str(e)}[/red]")
    ...
    def show_config_menu(self):
        ...
        table = Table(title="Configuraci�n Actual", show_header=True, header_style="bold magenta")
        ...
        table.add_row("TICKER", Config.TICKER, "S�mbolo por defecto")
        table.add_row("PERIOD", Config.PERIOD, "Per�odo de datos")
        table.add_row("INTERVAL", Config.INTERVAL, "Intervalo de tiempo")
        table.add_row("PREDICTION_DAYS", str(Config.PREDICTION_DAYS), "D�as a predecir")
    ...
    def show_available_symbols(self):
        ...
        self.console.print("\n[bold cyan]📊 FUTUROS DE �NDICES DISPONIBLES:[/bold cyan]\n")
        
        futures_table = Table(show_header=True, header_style="bold green")
        futures_table.add_column("S�mbolo", style="cyan", width=10)
        futures_table.add_column("Descripci�n", style="white")
        ...
        stocks_table.add_column("S�mbolo", style="cyan", width=10)
        stocks_table.add_column("Descripci�n", style="white")
